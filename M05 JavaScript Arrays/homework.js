...
function incrementarPorUno(array) {
   ...
   // Tu código:
   let incrementado = array.map((array) => {return array + 1});
   return incrementado;
}
...
function empiezaConNueve(num) {
   ...
   const respuesta = Array.from(resp);
   if (respuesta[0] === "9") {
      return true;
   } else {
      return false;
...
function mesesDelAño(array) {
   ...
   if (nuevosMeses.includes("Enero") && nuevosMeses.includes("Marzo") && nuevosMeses.includes("Noviembre")) {
      return nuevosMeses;
   } else {
...
function mayorACien(array) {
   ...
   for (let i = 0; i < array.length; i++) {
      if (array[i]>100){
         arregloCon100.push(array[i]);
      }
   }
   return arregloCon100;
}
